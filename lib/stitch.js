...
var updateRules = function(sheets) {
    return sheets.reduce(function(rules, sheet) {
        sheet.sprites.some(function(sprite) {
...
function stitch(filename, targetPath, urlFilter) {
    //console.log('Stiching %s to %s', filename, targetPath);

    var ast = css.parse(fs.readFileSync(filename, 'utf-8'));

    // 1. Find valid background rules
    var backgrounds = findBackgrounds(ast.stylesheet.rules, urlFilter);
    //console.log('Found %d backgrounds matching %s', backgrounds.length, urlFilter);

    // 3. Group by target url
    var groups = groupBackgrounds(backgrounds, path.basename(filename, '.css'));
    //console.log('Created %d groups', Object.keys(groups).length);

    // 4. Create sprite sheets
    var sheets = createSpriteSheets(groups, path.dirname(filename), targetPath);
    //console.log('Created %d sheets at %s', sheets.length, targetPath);

    // 5. Update rules
    var rules = updateRules(sheets);
    //console.log('Updated %d rules', rules.length);

    return css.stringify(ast);
}

var filename = '/Users/simen/Projects/CLP/css/base.css';
var result = stitch(filename, 'images/generated/sprites', 'images/sprites');
